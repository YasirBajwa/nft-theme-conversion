import React from "react";
import AvatarImage21 from "./../utils/img/avatars/avatar_11.png";
import { Header } from "./header";

export const LoginPage = () => {
  return (
    <>
      <div className="overflow-hidden">
        <Header />
        <div className="edit_profile register login">
          <div className="container">
            <div className="row">
              <div className="col-lg-7" />
              <div className="col-lg-5">
                <div className="right_part space-y-20">
                  <h1 className="color_white"> Welcome to Raroin </h1>
                  <p
                    className="color_white"
                    style={{ color: "white !important" }}
                  >
                    Don't have an account yet?{" "}
                    <a > Register </a>
                  </p>
                  <div className="box edit_box w-full space-y-20">
                    <div className="space-y-10">
                      <span className="nameInput">Email </span>
                      <div className="confirm">
                        <input
                          type="text"
                          className="form-control"
                          placeholder="Enter email"
                        />
                      </div>
                    </div>
                    <div className="space-y-10">
                      <span className="nameInput">Password</span>
                      <div className="confirm">
                        <input
                          type="password"
                          className="form-control"
                          placeholder="Enter your password"
                        />
                      </div>
                    </div>
                    <a className="btn btn-white btn-sm color_green">
                      {" "}
                      Click to verify{" "}
                    </a>
                    <a  className="btn btn-grad w-full btn-lg ">
                      {" "}
                      Create an account
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
